...
#define GETVAL_CNT 10
vu16 tpad_default_val=0;//空载的时候(没有手按下),计数器需要的时间


//初始化触摸按键
//获得空载的时候触摸按键的取值.提供给TPAD_Scan使用
//返回值:0,初始化成功;1,初始化失败
u8 TPAD_Init(u8 psc)
{
	u16 buf[10];
	...
	for (i=0;i < GETVAL_CNT;i++) { //连续读取10次
		buf[i]= TPAD_Get_Val();;
		delay_ms(10);	    
	}				    
	for (i=0;i< GETVAL_CNT-1;i++) { //冒泡排序
		for (j=i+1; j < GETVAL_CNT; j++) {
			if(buf[i] > buf[j]) { //升序排列
				temp = buf[i];
				buf[i] = buf[j];
				buf[j] = temp;
	...
	temp=0;
	for (i=2; i<8; i++) {
		temp+=buf[i];//取中间的6个数据进行平均，保证值比较稳定
	}
...
u8 TPAD_Scan(u8 mode)
{
	...
	// keyen>0 return res==0 无效
	// keyen==0 return res==1 有效
	return res; 
}	

//定时器2通道2输入捕获配置，没有使用中断
