...
#define	FCS_SET  	    GPIO_SetBits(GPIOB,GPIO_Pin_12)     //片选对应
#define	FCS_CLR  	    GPIO_ResetBits(GPIOB,GPIO_Pin_12)   //片选对应

#define	FDI_SET       GPIO_SetBits(GPIOB,GPIO_Pin_15)      //数据输出
#define	FDI_CLR       GPIO_ResetBits(GPIOB,GPIO_Pin_15)    //数据输出

#define	FDO_IN        GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_14)   //数据输入


#define	FCLK_SET      GPIO_SetBits(GPIOB,GPIO_Pin_13)   //数据时钟线

#define	FCLK_CLR      GPIO_ResetBits(GPIOB,GPIO_Pin_13) //数据时钟线
...
#define	Font_SPI_WR    LCD_SPI_ReadWriteByte    //使用液晶屏的SPI通信方式
